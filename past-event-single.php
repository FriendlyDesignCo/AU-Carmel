<?php
  // Variable setup for this page.
  // If and when this template is transferred to a CMS,
  // these variables should hopefully be replaced by
  // something CMS-driven.

  $currentPage = 'events';
?>

<!DOCTYPE html>
<html lang="en">
  <head>
    
    <?php include('includes/head.php'); ?>

    <title>The Strength of Cooperation: Lessons from the Grand Alliance 1941-1945 – The Carmel Institute of Russian Culture &amp; History</title>

  </head>
  <body class="<?php echo $currentPage; ?>">
    
    <?php include('includes/site-header.php'); ?>

    <article>
      <!-- BEGIN PAGE TITLE -->
      <header class="page-title page-title--has-image">
        <div class="page-title__background" style="background-image: url(img/events/2015-symposium/20150411CISymp06.jpg);"></div>
        <div class="container">
          <h1>The Strength of Cooperation: Lessons from the Grand Alliance 1941-1945</h1>
          <div class="page-title__info-container">
            <span class="page-title__info">April 11, 2015</span>
            <span class="page-title__info">555 11th St NW<br />Washington, DC 20004</span>
          </div>
        </div>
      </header>
      <!-- END PAGE TITLE -->

      <!-- BEGIN TWO COLUMN CONTENT AREA -->
      <div class="content-area content-area--two-col  container">
        <div class="content-area__column content-area__column--wide">
          
          <div class="slideshow-controls">
            <a class="slideshow-controls__prev">&lt;</a>
            <a class="slideshow-controls__next">&gt;</a>
          </div>

          <div class="cycle-slideshow" data-cycle-slides=".cycle-slideshow__slide" data-cycle-timeout="0" data-cycle-prev=".slideshow-controls__prev" data-cycle-next=".slideshow-controls__next">

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp01.jpg" alt="Ambassador Sergey I. Kislyak and Susan B. Eisenhower" />
              <p class="cycle-slideshow__caption">Ambassador Sergey I. Kislyak and Susan B. Eisenhower</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp02.jpg" alt="Ambassador Sergey I. Kislyak" />
              <p class="cycle-slideshow__caption">Ambassador Sergey I. Kislyak</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp03.jpg" alt="Former Ambassador John R. Beyrle" />
              <p class="cycle-slideshow__caption">Former Ambassador John R. Beyrle</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp04.jpg" alt="Dr. Anton Fedyashin, Dr. Iskander Magadeev, Dr. Frank Costigliola, Susan B. Eisenhower, Ambassador John R. Beyrle" />
              <p class="cycle-slideshow__caption">Dr. Anton Fedyashin, Dr. Iskander Magadeev, Dr. Frank Costigliola, Susan B. Eisenhower, Ambassador John R. Beyrle</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp05.jpg" alt="Dr. Iskander Magadeev, Susan B. Eisenhower, Dr. Anton Fedyashin, Susan Carmel Lehrman, Ambassador John R. Beyrle, Dr. Frank Costigliola" />
              <p class="cycle-slideshow__caption">Dr. Iskander Magadeev, Susan B. Eisenhower, Dr. Anton Fedyashin, Susan Carmel Lehrman, Ambassador John R. Beyrle, Dr. Frank Costigliola</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp06.jpg" alt="Susan B. Eisenhower, Dr. Frank Costigliola, Susan Carmel Lehrman, former Ambassador John R. Beyrle, Dr. Iskander Magadeev, (background) Susan Carmel Lehrman Scholarship Students" />
              <p class="cycle-slideshow__caption">Susan B. Eisenhower, Dr. Frank Costigliola, Susan Carmel Lehrman, former Ambassador John R. Beyrle, Dr. Iskander Magadeev, (background) Susan Carmel Lehrman Scholarship Students</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp07.jpg" alt="Susan Carmel Lehrman and Dr. Iskander Magadeev" />
              <p class="cycle-slideshow__caption">Susan Carmel Lehrman and Dr. Iskander Magadeev</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp08.jpg" alt="Susan B. Eisenhower" />
              <p class="cycle-slideshow__caption">Susan B. Eisenhower</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp10.jpg" alt="Susan Carmel Lehrman and Dr. Iskander Magadeev" />
              <p class="cycle-slideshow__caption">Susan Carmel Lehrman and Dr. Iskander Magadeev</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp11.jpg" alt="Guests mingle at the reception." />
              <p class="cycle-slideshow__caption">Guests mingle at the reception.</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp12.jpg" alt="Panelists and audience at "The Strength of Cooperation" symposium." />
              <p class="cycle-slideshow__caption">Panelists and audience at "The Strength of Cooperation" symposium.</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp14.jpg" alt="Guests at the reception." />
              <p class="cycle-slideshow__caption">Guests at the reception.</p>
            </div>

            <div class="cycle-slideshow__slide">
              <img src="img/events/2015-symposium/20150411CISymp15.jpg" alt="Dr. Iskander Magadeev and guest" />
              <p class="cycle-slideshow__caption">Dr. Iskander Magadeev and guest</p>
            </div>

          </div> <!-- .cycle-slideshow -->

          <div class="clearfix"></div>
        </div> <!-- .content-area__column wide -->
        <aside class="content-area__column content-area__column--narrow">
          <h3 class="content-area__title">
            <span>About the Symposium</span>
          </h3>
          <p>The Carmel Institute of Russian Culture and History's 2015 symposium celebrated the 70th anniversary of the Grand Alliance between the United States, Russia and Great Britain in World War II with experts on Roosevelt, Stalin, Churchill, Eisenhower and the amazing story of the one American GI known to have fought with both U.S. forces and the Red Army.</p>
          
        </aside> <!-- .content-area__column narrow -->
      </div>
      <!-- END TWO COLUMN CONTENT AREA -->
    </article>


    <?php include('includes/site-footer.php'); ?>

  </body>
</html>